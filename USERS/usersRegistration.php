...
<body class="bg-light">
    <div class="container">
    	<h1 class="text-center ">User Registraion </h1>
    	<!-- Form --> 
    	<form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-outline mb-4">
                ...
                <input type=" Password" name=" Confirm Password" id=" Confirm Password"placeholder= " Confirm Password" class="form-control" autocomplete="off">
            </div>
            ...
            <p class=" fw-bold mt-2 pt-1 mb-0">Already Have an account ? <a href="http://localhost/Hidden%20Store/cheackout.php" class="text-danger">Login</a></p>
         	</div>
    	</form>
    </div>
</body>
...
<?php 
 session_start();
  ...
  if (isset($_POST['submit'])) {
       ...
       $IsExist=$db->prepare("SELECT * FROM users WHERE USERNAME=:USERNAME");
       ...
       $reqister=$db->prepare("INSERT INTO users (USERNAME,user_Email,user_password,user_image,user_ip,user_Addres,user_mobile) VALUES (:USERNAME,:user_Email,:user_password,:user_image,:user_ip,:user_Addres,:user_mobile)");
        ...
        $reqister->bindparam("user_Email",$_POST['Email']);
         $reqister->bindparam("user_password",$$_POST['Password']);
          $reqister->bindparam("user_image",$image);
           $reqister->bindparam("user_ip", $ip_server);
            $reqister->bindparam("user_Addres",$_POST['Address']);
             $reqister->bindparam("user_mobile",$_POST['mobile']);
             $reqister->execute();
           }
           $update=$db->prepare("SELECT * FROM cart_details WHERE ipAddress=:PU ");

       $update->bindparam("PU",$ip_server);
       $update->execute();
            if ($update->rowcount()>0) { 
                $_SESSION['USERNAME']==$_POST['USERNAME'];
                 echo "<script> alert('Regiter is Done ')</script>";
            }
  }
...
?>
        <?php 
         ...
         if(isset($_POST['submit'])){
             $InsertPlace=$db->prepare("INSERT INTO place (images ,about ,Governorate, area , location ,capacity ,minBudget ,phone ,email) VALUES (:images,:about ,:Governorate, :area  ,:location ,:capacity  ,:minBudget ,:phone ,:email)");
              ...
              $InsertPlace->bindparam("about",$_POST['About']);
               $InsertPlace->bindparam("Governorate",$_POST['Governorate']);
                $InsertPlace->bindparam("area",$_POST['Area']);
                   ...
                   $InsertPlace->bindparam("phone",$_POST['SELECT']);
