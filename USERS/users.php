...
<?php 
  ...
  if (isset($_POST['submit'])) {
    ...
    $Login=$db->prepare("SELECT * FROM users WHERE USERNAME =:USERNAME And user_password=:PU ");
    $Login->bindparam("USERNAME",$_POST['USERNAME']);
     $Login->bindparam("PU",$_POST['Password']);
    $Login->execute();
    $selecyCart=$db->prepare("SELECT * FROM cart_details WHERE ipAddress-:IP");
    ...
    if ($Login->rowcount()==1 ){
            ...
            echo "<script> alert('succesful')</script>"; 
                echo "<script> window.open('http://localhost/Hidden%20Store/USERS/Payment.php?user= ','_self')</script>"; 
              }
                // code...
              }
